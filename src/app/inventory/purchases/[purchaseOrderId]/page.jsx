...
export default function DetailPurchasePage() {
  ...
  const purchaseOrder = {
    ...
    paymentStatus: "Lunas",
    items: [
      { id: 1, name: "Kertas A4 80gsm", qty: 10, unitPrice: 45000 },
      { id: 2, name: "Tinta Printer Hitam", qty: 5, unitPrice: 75000 },
      { id: 3, name: "Stapler Besar", qty: 2, unitPrice: 50000 },
    ],
  };
                  ...
                  <tbody>
                    {PO?.purchaseOrderItems?.map((item, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border-b">{`${item.product.name} - ${item.product.brand} - ${item.product.type} - ${item.product.size}`}</td>
